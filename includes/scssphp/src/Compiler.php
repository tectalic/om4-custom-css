...
use OM4\Vendor\ScssPhp\ScssPhp\Colors;
use OM4\Vendor\ScssPhp\ScssPhp\Compiler\Environment;
use OM4\Vendor\ScssPhp\ScssPhp\Exception\CompilerException;
use OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock;
use OM4\Vendor\ScssPhp\ScssPhp\Node;
use OM4\Vendor\ScssPhp\ScssPhp\SourceMap\SourceMapGenerator;
use OM4\Vendor\ScssPhp\ScssPhp\Type;
use OM4\Vendor\ScssPhp\ScssPhp\Parser;
use OM4\Vendor\ScssPhp\ScssPhp\Util;
/**
...
class Compiler
{
    ...
    public static $true = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, 'true'];
    public static $false = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, 'false'];
    public static $NaN = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, 'NaN'];
    public static $Infinity = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, 'Infinity'];
    public static $null = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_NULL];
    public static $nullString = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', []];
    public static $defaultValue = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, ''];
    public static $selfSelector = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_SELF];
    public static $emptyList = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, '', []];
    public static $emptyMap = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_MAP, [], []];
    public static $emptyString = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '"', []];
    public static $with = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, 'with'];
    public static $without = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, 'without'];
    protected $importPaths = [''];
    protected $importCache = [];
    protected $importedFiles = [];
    protected $userFunctions = [];
    protected $registeredVars = [];
    protected $registeredFeatures = ['extend-selector-pseudoclass' => \false, 'at-error' => \true, 'units-level-3' => \false, 'global-variable-shadowing' => \false];
     ...
     */
    protected $formatter = 'OM4\\Vendor\\ScssPhp\\ScssPhp\\Formatter\\Nested';
    protected $rootEnv;
    protected $rootBlock;
    /**
    ...
    public function __construct($cacheOptions = null)
    {
        ...
        $this->sourceNames = [];
        if ($cacheOptions) {
            $this->cache = new \OM4\Vendor\ScssPhp\ScssPhp\Cache($cacheOptions);
        }
    ...
    {
        $options = ['importPaths' => $this->importPaths, 'registeredVars' => $this->registeredVars, 'registeredFeatures' => $this->registeredFeatures, 'encoding' => $this->encoding, 'sourceMap' => \serialize($this->sourceMap), 'sourceMapOptions' => $this->sourceMapOptions, 'formatter' => $this->formatter];
        return $options;
    }
    /**
    ...
    public function compile($code, $path = null)
    {
        ...
        if ($this->sourceMap) {
            if (\is_object($this->sourceMap) && $this->sourceMap instanceof \OM4\Vendor\ScssPhp\ScssPhp\SourceMap\SourceMapGenerator) {
                $sourceMapGenerator = $this->sourceMap;
                $this->sourceMap = self::SOURCE_MAP_FILE;
            } elseif ($this->sourceMap !== self::SOURCE_MAP_NONE) {
                $sourceMapGenerator = new \OM4\Vendor\ScssPhp\ScssPhp\SourceMap\SourceMapGenerator($this->sourceMapOptions);
            }
        }
        ...
        if (!empty($out) && $this->sourceMap && $this->sourceMap !== self::SOURCE_MAP_NONE) {
            ...
            switch ($this->sourceMap) {
                case self::SOURCE_MAP_INLINE:
                    $sourceMapUrl = \sprintf('data:application/json,%s', \OM4\Vendor\ScssPhp\ScssPhp\Util::encodeURIComponent($sourceMap));
                    break;
     ...
     * @param string $path
     *
     * @return \OM4\Vendor\ScssPhp\ScssPhp\Parser
     */
    protected function parserFactory($path)
    {
        ...
        $parser = new \OM4\Vendor\ScssPhp\ScssPhp\Parser($path, \count($this->sourceNames), $this->encoding, $this->cache, $cssOnly);
     ...
     *
     * @return \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock
     */
    protected function makeOutputBlock($type, $selectors = null)
    {
        $out = new \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock();
        $out->type = $type;
        $out->lines = [];
        $out->children = [];
        ...
        if ($this->env->block instanceof \OM4\Vendor\ScssPhp\ScssPhp\Block) {
     ...
     * Compile root
     *
     * @param \OM4\Vendor\ScssPhp\ScssPhp\Block $rootBlock
     */
    protected function compileRoot(\OM4\Vendor\ScssPhp\ScssPhp\Block $rootBlock)
    {
        $this->rootBlock = $this->scope = $this->makeOutputBlock(\OM4\Vendor\ScssPhp\ScssPhp\Type::T_ROOT);
    ...
    protected function missingSelectors()
    {
        foreach ($this->extends as $extend) {
            ...
            $this->sourceLine = $block[\OM4\Vendor\ScssPhp\ScssPhp\Parser::SOURCE_LINE];
     ...
     */
    protected function flattenSelectors(\OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $block, $parentKey = null)
    {
        if ($block->selectors) {
            $selectors = [];
     ...
     * Compile media
     *
     * @param \OM4\Vendor\ScssPhp\ScssPhp\Block $media
     */
    protected function compileMedia(\OM4\Vendor\ScssPhp\ScssPhp\Block $media)
    {
        $this->pushEnv($media);
        ...
        if (!empty($mediaQueries) && $mediaQueries) {
            ...
            foreach ($mediaQueries as $mediaQuery) {
                $this->scope = $this->makeOutputBlock(\OM4\Vendor\ScssPhp\ScssPhp\Type::T_MEDIA, [$mediaQuery]);
            ...
            foreach ($media->children as $child) {
                $type = $child[0];
                if ($type !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_BLOCK && $type !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MEDIA && $type !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_DIRECTIVE && $type !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_IMPORT) {
                    $needsWrap = \true;
                    break;
                }
            }
            if ($needsWrap) {
                $wrapped = new \OM4\Vendor\ScssPhp\ScssPhp\Block();
                ...
                $media->children = [[\OM4\Vendor\ScssPhp\ScssPhp\Type::T_BLOCK, $wrapped]];
                if (isset($this->lineNumberStyle)) {
                    $annotation = $this->makeOutputBlock(\OM4\Vendor\ScssPhp\ScssPhp\Type::T_COMMENT);
                    $annotation->depth = 0;
     ...
     * Media parent
     *
     * @param \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $scope
     *
     * @return \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock
     */
    protected function mediaParent(\OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $scope)
    {
        while (!empty($scope->parent)) {
            if (!empty($scope->type) && $scope->type !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MEDIA) {
                break;
            }
            $scope = $scope->parent;
        }
     ...
     * Compile directive
     *
     * @param \OM4\Vendor\ScssPhp\ScssPhp\Block|array $block
     * @param \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $out
     */
    protected function compileDirective($directive, \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $out)
    {
        if (\is_array($directive)) {
            ...
            if (empty($directive[3])) {
                $this->appendRootDirective($s . ';' . $appendNewLine, $out, [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_COMMENT, \OM4\Vendor\ScssPhp\ScssPhp\Type::T_DIRECTIVE]);
            } else {
                $this->appendOutputLine($out, \OM4\Vendor\ScssPhp\ScssPhp\Type::T_DIRECTIVE, $s . ';');
            }
        } else {
     ...
     * Compile at-root
     *
     * @param \OM4\Vendor\ScssPhp\ScssPhp\Block $block
     */
    protected function compileAtRoot(\OM4\Vendor\ScssPhp\ScssPhp\Block $block)
    {
        ...
        if ($block->selector) {
            ...
            $block->children = [[\OM4\Vendor\ScssPhp\ScssPhp\Type::T_BLOCK, $wrapped]];
            $block->selector = null;
        }
    ...
    protected function filterScopeWithWithout($scope, $with, $without)
    {
        $filteredScopes = [];
        $childStash = [];
        if ($scope->type === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_ROOT) {
            return $scope;
        }
        // start from the root
        while ($scope->parent && $scope->parent->type !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_ROOT) {
        ...
        for (;;) {
            ...
            if ($this->isWith($scope, $with, $without)) {
                ...
                $s->lines = [];
                $s->parent = null;
                if ($s->type !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MEDIA && $s->type !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_DIRECTIVE) {
                    $s->selectors = [];
                }
                $filteredScopes[] = $s;
            }
     ...
     *
     * @param \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $scope
     * @param \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $previousScope
     *
     * @return mixed
     */
    ...
    protected function compileWith($withCondition)
    {
        ...
        $without = ['rule' => \true];
        if ($withCondition) {
            if ($withCondition[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_INTERPOLATE) {
     ...
     * @param array $without
     *
     * @return \OM4\Vendor\ScssPhp\ScssPhp\Compiler\Environment
     */
     ...
     * Filter WITH rules
     *
     * @param \OM4\Vendor\ScssPhp\ScssPhp\Block|\ScssPhp\ScssPhp\Formatter\OutputBlock $block
     * @param array                                                         $with
     * @param array                                                         $without
     *
     * @return boolean
    ...
    {
        if (isset($block->type)) {
            if ($block->type === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MEDIA) {
            ...
            if (\count($block->selectors)) {
                ...
                if (\is_object($s) && $s instanceof \OM4\Vendor\ScssPhp\ScssPhp\Node\Number) {
     ...
     * @param array                  $selectors
     */
    protected function compileKeyframeBlock(\OM4\Vendor\ScssPhp\ScssPhp\Block $block, $selectors)
    {
        $env = $this->pushEnv($block);
        $envs = $this->compactEnv($env);
        $this->env = $this->extractEnv(\array_filter($envs, function (\OM4\Vendor\ScssPhp\ScssPhp\Compiler\Environment $e) {
     ...
     * @param \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $out
     */
    protected function compileNestedPropertiesBlock(\OM4\Vendor\ScssPhp\ScssPhp\Block $block, \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $out)
    {
        ...
        foreach ($block->children as $child) {
            switch ($child[0]) {
                case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_ASSIGN:
                    ...
                    break;
                case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NESTED_PROPERTY:
     ...
     * @param array                  $selectors
     */
    protected function compileNestedBlock(\OM4\Vendor\ScssPhp\ScssPhp\Block $block, $selectors)
    {
        $this->pushEnv($block);
        $this->scope = $this->makeOutputBlock($block->type, $selectors);
        $this->scope->parent->children[] = $this->scope;
        ...
        if ($block->type !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_DIRECTIVE || $this->compileDirectiveName($block->name) !== 'font-face') {
            // need wrapping?
            $needWrapping = \false;
            foreach ($block->children as $child) {
                if ($child[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_ASSIGN) {
                    $needWrapping = \true;
                    break;
                }
            }
     ...
     * @param \OM4\Vendor\ScssPhp\ScssPhp\Block $block
     */
    protected function compileBlock(\OM4\Vendor\ScssPhp\ScssPhp\Block $block)
    {
        $env = $this->pushEnv($block);
    ...
    protected function compileCommentValue($value, $pushEnv = \false)
    {
        ...
        if (isset($value[2])) {
            ...
            $this->ignoreCallStackMessage = \true;
            try {
    ...
    {
        $out = $this->makeOutputBlock(\OM4\Vendor\ScssPhp\ScssPhp\Type::T_COMMENT);
    ...
    {
        foreach ($part as &$p) {
            if (\is_array($p) && ($p[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_INTERPOLATE || $p[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING)) {
    ...
    protected function collapseSelectors($selectors, $selectorFormat = \false)
    {
        ...
        foreach ($selectors as $selector) {
            ...
            if ($selectorFormat) {
                foreach ($output as &$o) {
                    $o = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', [$o]];
                }
                $output = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, ' ', $output];
            } else {
        ...
        if ($selectorFormat) {
            $parts = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, ',', $parts];
        } else {
            $parts = \implode(', ', $parts);
        }
        return $parts;
    ...
    protected function replaceSelfSelector($selectors, $replace = null)
    {
        foreach ($selectors as &$part) {
            if (\is_array($part)) {
                if ($part === [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_SELF]) {
                    if (\is_null($replace)) {
                        $replace = $this->reduce([\OM4\Vendor\ScssPhp\ScssPhp\Type::T_SELF]);
    ...
    protected function compileSelectorPart($piece)
    {
        foreach ($piece as &$p) {
            ...
            switch ($p[0]) {
                case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_SELF:
                    $p = '&';
                    break;
                default:
                    $p = $this->compileValue($p);
    ...
    {
        $this->callStack[] = ['n' => $name, \OM4\Vendor\ScssPhp\ScssPhp\Parser::SOURCE_INDEX => $this->sourceIndex, \OM4\Vendor\ScssPhp\ScssPhp\Parser::SOURCE_LINE => $this->sourceLine, \OM4\Vendor\ScssPhp\ScssPhp\Parser::SOURCE_COLUMN => $this->sourceColumn];
     ...
     * @return array|null
     */
    protected function compileChildren($stms, \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $out, $traceName = '')
    {
     ...
     * @param array                                  $stms
     * @param \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $out
     * @param \OM4\Vendor\ScssPhp\ScssPhp\Block                 $selfParent
     * @param string                                 $traceName
     *
     * @throws \Exception
     */
    protected function compileChildrenNoReturn($stms, \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $out, $selfParent = null, $traceName = '')
    {
        $this->pushCallStack($traceName);
        foreach ($stms as $stm) {
            if ($selfParent && isset($stm[1]) && \is_object($stm[1]) && $stm[1] instanceof \OM4\Vendor\ScssPhp\ScssPhp\Block) {
                $stm[1]->selfParent = $selfParent;
                $ret = $this->compileChild($stm, $out);
                $stm[1]->selfParent = null;
            } elseif ($selfParent && \in_array($stm[0], [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_INCLUDE, \OM4\Vendor\ScssPhp\ScssPhp\Type::T_EXTEND])) {
        ...
        foreach ($queryList as $kql => $query) {
            ...
            foreach ($query as $kq => $q) {
                for ($i = 1; $i < \count($q); $i++) {
                    ...
                    if ($q[0] == \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MEDIA_TYPE && (\strpos($value, '(') !== \false || \strpos($value, ')') !== \false || \strpos($value, ':') !== \false || \strpos($value, ',') !== \false)) {
                        $shouldReparse = \true;
                    }
                    $queryList[$kql][$kq][$i] = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, $value];
                }
            }
            if ($shouldReparse) {
                if (\is_null($parser)) {
    ...
    protected function compileMediaQuery($queryList)
    {
        ...
        foreach ($queryList as $query) {
            ...
            $mediaTypeOnly = \true;
            foreach ($query as $q) {
                if ($q[0] !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MEDIA_TYPE) {
                    $mediaTypeOnly = \false;
                    break;
                }
            }
            foreach ($query as $q) {
                switch ($q[0]) {
                    case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MEDIA_TYPE:
                        ...
                        if (!$mediaTypeOnly) {
                            if (\in_array(\OM4\Vendor\ScssPhp\ScssPhp\Type::T_NOT, $newType) || $type && \in_array(\OM4\Vendor\ScssPhp\ScssPhp\Type::T_NOT, $type)) {
                                if ($type) {
                        ...
                        break;
                    case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MEDIA_EXPRESSION:
                        if (isset($q[2])) {
                        ...
                        break;
                    case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MEDIA_VALUE:
                        $parts[] = $this->compileValue($q[1]);
                        break;
                }
            }
    ...
    protected function mergeMediaTypes($type1, $type2)
    {
        ...
        if ($m1 === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NOT ^ $m2 === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NOT) {
            if ($t1 === $t2) {
                return null;
            }
            return [$m1 === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NOT ? $m2 : $m1, $m1 === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NOT ? $t2 : $t1];
        }
        if ($m1 === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NOT && $m2 === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NOT) {
            ...
            return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_NOT, $t1];
        }
        if ($t1 !== $t2) {
            return null;
        }
     ...
     * @return boolean
     */
    protected function compileImport($rawPath, \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $out, $once = \false)
    {
        if ($rawPath[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING) {
        ...
        if ($rawPath[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST) {
            // handle a list of strings
            if (\count($rawPath[2]) === 0) {
                return \false;
            }
            foreach ($rawPath[2] as $path) {
                if ($path[0] !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING) {
                    $this->appendRootDirective('@import ' . $this->compileImportPath($rawPath) . ';', $out);
                    return \false;
                }
            }
    ...
    {
        switch ($path[0]) {
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST:
            ...
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING:
                if ($path[1]) {
                    ...
                    $path = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, $path];
                }
                break;
        }
        return $path;
     ...
     *
     * @param string                                        $line
     * @param @param \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $out
     * @param array                                         $allowed
     */
    protected function appendRootDirective($line, $out, $allowed = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_COMMENT])
    {
        $root = $out;
        while ($root->parent) {
            $root = $root->parent;
        ...
        $child = $this->makeOutputBlock(\OM4\Vendor\ScssPhp\ScssPhp\Type::T_COMMENT);
        $child->lines[] = $line;
     ...
     */
    protected function appendOutputLine(\OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $out, $type, $line)
    {
        $outWrite =& $out;
     ...
     * @return array
     */
    protected function compileChild($child, \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $out)
    {
        if (isset($child[\OM4\Vendor\ScssPhp\ScssPhp\Parser::SOURCE_LINE])) {
            $this->sourceIndex = isset($child[\OM4\Vendor\ScssPhp\ScssPhp\Parser::SOURCE_INDEX]) ? $child[\OM4\Vendor\ScssPhp\ScssPhp\Parser::SOURCE_INDEX] : null;
            $this->sourceLine = isset($child[\OM4\Vendor\ScssPhp\ScssPhp\Parser::SOURCE_LINE]) ? $child[\OM4\Vendor\ScssPhp\ScssPhp\Parser::SOURCE_LINE] : -1;
            $this->sourceColumn = isset($child[\OM4\Vendor\ScssPhp\ScssPhp\Parser::SOURCE_COLUMN]) ? $child[\OM4\Vendor\ScssPhp\ScssPhp\Parser::SOURCE_COLUMN] : -1;
        ...
        switch ($child[0]) {
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_SCSSPHP_IMPORT_ONCE:
                $rawPath = $this->reduce($child[1]);
                $this->compileImport($rawPath, $out, \true);
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_IMPORT:
                $rawPath = $this->reduce($child[1]);
            ...
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_DIRECTIVE:
                ...
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_AT_ROOT:
                $this->compileAtRoot($child[1]);
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MEDIA:
                $this->compileMedia($child[1]);
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_BLOCK:
                $this->compileBlock($child[1]);
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_CHARSET:
                if (!$this->charsetSeen) {
                    $this->charsetSeen = \true;
                    $this->appendRootDirective('@charset ' . $this->compileValue($child[1]) . ';', $out);
                }
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_CUSTOM_PROPERTY:
                list(, $name, $value) = $child;
                $compiledName = $this->compileValue($name);
                // if the value reduces to null from something else then
                // the property should be discarded
                if ($value[0] !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NULL) {
                    $value = $this->reduce($value);
                    if ($value[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NULL || $value === static::$nullString) {
                        break;
                    }
                }
                $compiledValue = $this->compileValue($value);
                $line = $this->formatter->customProperty($compiledName, $compiledValue);
                $this->appendOutputLine($out, \OM4\Vendor\ScssPhp\ScssPhp\Type::T_ASSIGN, $line);
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_ASSIGN:
                list(, $name, $value) = $child;
                if ($name[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_VARIABLE) {
                ...
                if (\in_array($compiledName, ['font', 'grid-row', 'grid-column', 'border-radius'])) {
                    if ($value[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_VARIABLE) {
                    ...
                    if ($compiledName === 'font' && $value[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST && $value[1] === ',') {
                    ...
                    if ($shorthandValue[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_EXPRESSION && $shorthandValue[1] === '/') {
                        $revert = \true;
                    ...
                    } elseif ($shorthandValue[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST) {
                        foreach ($shorthandValue[2] as &$item) {
                            if ($item[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_EXPRESSION && $item[1] === '/') {
            ...
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_COMMENT:
                if ($out->type === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_ROOT) {
                    $this->compileComment($child);
                    break;
                }
                $line = $this->compileCommentValue($child, \true);
                $this->appendOutputLine($out, \OM4\Vendor\ScssPhp\ScssPhp\Type::T_COMMENT, $line);
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MIXIN:
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION:
                ...
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_EXTEND:
                ...
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_IF:
                list(, $if) = $child;
                if ($this->isTruthy($this->reduce($if->cond, \true))) {
                    return $this->compileChildren($if->children, $out);
                }
                foreach ($if->cases as $case) {
                    if ($case->type === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_ELSE || $case->type === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_ELSEIF && $this->isTruthy($this->reduce($case->cond))) {
                        return $this->compileChildren($case->children, $out);
                    }
                }
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_EACH:
                list(, $each) = $child;
                ...
                foreach ($list[2] as $item) {
                    ...
                    if ($ret) {
                        if ($ret[0] !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_CONTROL) {
                ...
                $this->backPropagateEnv($store, $each->vars);
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_WHILE:
                list(, $while) = $child;
                ...
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_FOR:
                list(, $for) = $child;
                $start = $this->reduce($for->start, \true);
                $end = $this->reduce($for->end, \true);
                if (!$start instanceof \OM4\Vendor\ScssPhp\ScssPhp\Node\Number) {
                    throw $this->error('%s is not a number', $start[0]);
                }
                if (!$end instanceof \OM4\Vendor\ScssPhp\ScssPhp\Node\Number) {
                ...
                for (;;) {
                    ...
                    $this->set($for->var, new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number($start, $unit));
                    $start += $d;
                ...
                $this->backPropagateEnv($store, [$for->var]);
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_BREAK:
                return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_CONTROL, \true];
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_CONTINUE:
                return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_CONTROL, \false];
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_RETURN:
                return $this->reduce($child[1], \true);
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NESTED_PROPERTY:
                $this->compileNestedPropertiesBlock($child[1], $out);
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_INCLUDE:
                // including a mixin
                ...
                if (isset($child['selfParent']) && isset($child['selfParent']->selectors)) {
                    ...
                    if ($parentSelectors) {
                        $parent = new \OM4\Vendor\ScssPhp\ScssPhp\Block();
                        $parent->selectors = $parentSelectors;
                        foreach ($mixin->children as $k => $child) {
                            if (isset($child[1]) && \is_object($child[1]) && $child[1] instanceof \OM4\Vendor\ScssPhp\ScssPhp\Block) {
                ...
                $this->popEnv();
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MIXIN_CONTENT:
                ...
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_DEBUG:
                list(, $value) = $child;
                ...
                break;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_WARN:
                list(, $value) = $child;
                $fname = $this->sourceNames[$this->sourceIndex];
                $line = $this->sourceLine;
                $value = $this->compileDebugValue($value);
    ...
    protected function expToString($exp, $keepParens = \false)
    {
        ...
        return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', $content];
    }
    /**
     * Is truthy?
     *
    ...
    protected function shouldEval($value)
    {
        switch ($value[0]) {
            ...
            // fall-thru
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_VARIABLE:
    ...
    protected function reduce($value, $inExp = \false)
    {
        ...
        switch ($value[0]) {
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_EXPRESSION:
                ...
                if ($opName == 'div' && !$inParens && !$inExp && isset($right[2]) && ($right[0] !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NUMBER && $right[2] != '' || $right[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NUMBER && !$right->unitless())) {
                ...
                if (\is_callable([$this, $fn]) || ($fn = "op{$ucLType}{$ucRType}") && \is_callable([$this, $fn]) && ($passOp = \true) || ($fn = "op{$ucOpName}") && \is_callable([$this, $fn]) && ($genOp = \true)) {
                    $coerceUnit = \false;
                    if (!isset($genOp) && $left[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NUMBER && $right[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NUMBER) {
                        $coerceUnit = \true;
                        switch ($opName) {
                            case 'mul':
                                $targetUnit = $left[2];
                        ...
                        if ($baseUnitLeft && $baseUnitRight && $baseUnitLeft === $baseUnitRight) {
                            ...
                            $left = new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number($left[1], []);
                        }
                    }
                    ...
                    if (isset($out)) {
                        if ($coerceUnit && $out[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NUMBER) {
            ...
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_UNARY:
                ...
                $exp = $this->reduce($exp);
                if ($exp[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NUMBER) {
                    switch ($op) {
                        case '+':
                            return new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number($exp[1], $exp[2]);
                        case '-':
                            return new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number(-$exp[1], $exp[2]);
                    }
                }
                ...
                return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', [$op, $exp]];
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_VARIABLE:
            ...
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST:
                ...
                return $value;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MAP:
            ...
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING:
                ...
                return $value;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_INTERPOLATE:
            ...
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_SELF:
                ...
                $selfSelector = $this->collapseSelectors($selfSelector, \true);
                return $selfSelector;
            default:
                return $value;
        }
    ...
    protected function fncall($functionReference, $argValues)
    {
        ...
        if (\is_string($functionReference)) {
            ...
            if ($functionReference === static::$null || $functionReference[0] !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION_REFERENCE) {
                $functionReference = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION, $name, [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, ',', []]];
            }
        }
        // a function type means we just want a plain css function call
        if ($functionReference[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION) {
            ...
            return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION, $functionReference[1], [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, ',', $listArgs]];
        }
        if ($functionReference === static::$null || $functionReference[0] !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION_REFERENCE) {
        ...
        switch ($functionReference[1]) {
            ...
            case 'native':
                ...
                if (\in_array($name, ['min', 'max'])) {
                    $cssFunction = $this->cssValidArg([\OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION_CALL, $name, $argValues], ['min', 'max', 'calc', 'env', 'var']);
                ...
                if (!isset($returnValue)) {
                    return $this->fncall([\OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION, $name, [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, ',', []]], $argValues);
                }
                return $returnValue;
            default:
                return static::$defaultValue;
    ...
    protected function cssValidArg($arg, $allowed_function = [], $inFunction = \false)
    {
        switch ($arg[0]) {
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_INTERPOLATE:
                return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, $this->CompileValue($arg)];
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION:
                if (!\in_array($arg[1], $allowed_function)) {
                    return \false;
                }
                if ($arg[2][0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST) {
            ...
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION_CALL:
                ...
                return $this->fncall([\OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION, $arg[1], [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, ',', []]], $cssArgs);
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING:
            ...
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST:
                ...
                if (empty($arg['enclosing']) and $arg[1] === '') {
                    ...
                    $arg[0] = \OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING;
                    return $arg;
                }
                return \false;
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_EXPRESSION:
        ...
        switch ($arg[0]) {
            ...
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION_CALL:
                ...
                if (\in_array($name, ['max', 'min', 'calc'])) {
                    $args = $arg[2];
                    $arg = $this->fncall([\OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION, $name, [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, ',', []]], $args);
                }
                break;
        }
        return $arg;
    ...
    protected function getFunctionReference($name, $safeCopy = \false)
    {
        ...
        if ($func = $this->get(static::$namespaces['function'] . $name, \false)) {
            ...
            return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION_REFERENCE, 'scss', $name, $func];
        }
        // native PHP functions
        ...
        if (isset($this->userFunctions[$normalizedName])) {
            ...
            return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION_REFERENCE, 'user', $name, $f, $prototype];
        }
        if (($f = $this->getBuiltinFunction($normalizedName)) && \is_callable($f)) {
            $libName = $f[1];
            $prototype = isset(static::${$libName}) ? static::${$libName} : null;
            return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION_REFERENCE, 'native', $name, $f, $prototype];
        }
        return static::$null;
    }
    /**
    ...
    public function normalizeValue($value)
    {
        ...
        switch ($value[0]) {
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST:
                ...
                if ($value[0] !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST) {
                    return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, $this->compileValue($value)];
                }
     ...
     * @param array $right
     *
     * @return \OM4\Vendor\ScssPhp\ScssPhp\Node\Number
     */
    protected function opAddNumberNumber($left, $right)
    {
        return new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number($left[1] + $right[1], $left[2]);
    }
    /**
     * Multiply numbers
     *
    ...
    protected function opAdd($left, $right)
    {
        if ($strLeft = $this->coerceString($left)) {
            if ($right[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING) {
                $right[1] = '';
            }
            $strLeft[2][] = $right;
            return $strLeft;
        ...
        if ($strRight = $this->coerceString($right)) {
            if ($left[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING) {
                $left[1] = '';
            }
    ...
    {
        $out = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_COLOR];
        foreach ([1, 2, 3] as $i) {
    ...
    {
        $value = $right[1];
        return $this->opColorColor($op, $left, [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_COLOR, $value, $value, $value]);
    }
    /**
    ...
    {
        $value = $left[1];
        return $this->opColorColor($op, [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_COLOR, $value, $value, $value], $right);
    }
    /**
    ...
    {
        $n = $left[1] - $right[1];
        return new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number($n ? $n / \abs($n) : 0, '');
    }
    /**
     * Cast to boolean
     *
    ...
    public function compileValue($value)
    {
        ...
        switch ($value[0]) {
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD:
                return $value[1];
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_COLOR:
                ...
                if (\count($value) === 5) {
                    ...
                    if (!\is_numeric($alpha) || $alpha < 1) {
                        $colorName = \OM4\Vendor\ScssPhp\ScssPhp\Colors::RGBaToColorName($r, $g, $b, $alpha);
                        if (!\is_null($colorName)) {
                            return $colorName;
                        }
                        if (\is_numeric($alpha)) {
                            $a = new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number($alpha, '');
                        } else {
                            $a = $alpha;
                        }
                        return 'rgba(' . $r . ', ' . $g . ', ' . $b . ', ' . $a . ')';
                ...
                $colorName = \OM4\Vendor\ScssPhp\ScssPhp\Colors::RGBaToColorName($r, $g, $b);
                if (!\is_null($colorName)) {
                    return $colorName;
                }
            ...
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING:
                ...
                if ($value[1]) {
                    ...
                    if ($value[1] === "'" && \strpos($content, '"') === \false && \strpbrk($content, '{}') !== \false) {
                        $value[1] = '"';
                    }
                    $content = \str_replace(array('\\a', "\n", "\f", '\\', "\r", $value[1]), array("\r", ' ', '\\f', '\\\\', '\\a', '\\' . $value[1]), $content);
                }
                return $value[1] . $content . $value[1];
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION:
                $args = !empty($value[2]) ? $this->compileValue($value[2]) : '';
                return "{$value[1]}({$args})";
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION_REFERENCE:
            ...
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST:
                ...
                foreach ($items as $item) {
                    if ($item[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NULL) {
                        continue;
                    }
            ...
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_INTERPOLATED:
            ...
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_INTERPOLATE:
                ...
                switch ($reduced[0]) {
                    case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST:
                        ...
                        if ($reduced[0] !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST) {
                            break;
                        }
                        ...
                        foreach ($items as $item) {
                            ...
                            $temp = $this->compileValue([\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, $item]);
                            if ($temp[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING) {
                                $filtered[] = $this->compileStringContent($temp);
                            } elseif ($temp[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD) {
                                $filtered[] = $temp[1];
                            } else {
                                $filtered[] = $this->compileValue($temp);
                            }
                        }
                        $reduced = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, \implode("{$delim}", $filtered)];
                        break;
                    case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING:
                        $reduced = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, $this->compileStringContent($reduced)];
                        break;
                    case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NULL:
                        $reduced = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, ''];
                }
    ...
    protected function extractInterpolation($list)
    {
        ...
        foreach ($items as $i => $item) {
            if ($item[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_INTERPOLATE) {
                $before = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, $list[1], \array_slice($items, 0, $i)];
                $after = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, $list[1], \array_slice($items, $i + 1)];
                return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_INTERPOLATED, $item, $before, $after];
            }
        }
        return $list;
    }
     ...
     *
     * @param \OM4\Vendor\ScssPhp\ScssPhp\Compiler\Environment $env
     * @param \OM4\Vendor\ScssPhp\ScssPhp\Block                $selfParent
     *
     * @return array
     */
    protected function multiplySelectors(\OM4\Vendor\ScssPhp\ScssPhp\Compiler\Environment $env, $selfParent = null)
    {
        $envs = $this->compactEnv($env);
        $selectors = [];
        $parentSelectors = [[]];
     ...
     * @return array
     */
    protected function multiplyMedia(\OM4\Vendor\ScssPhp\ScssPhp\Compiler\Environment $env = null, $childQueries = null)
    {
        if (!isset($env) || !empty($env->block->type) && $env->block->type !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MEDIA) {
            return $childQueries;
        }
        ...
        $parentQueries = isset($env->block->queryList) ? $env->block->queryList : [[[\OM4\Vendor\ScssPhp\ScssPhp\Type::T_MEDIA_VALUE, $env->block->value]]];
        ...
        if (\is_null($childQueries)) {
            ...
            foreach ($parentQueries as $parentQuery) {
                foreach ($originalQueries as $childQuery) {
                    $childQueries[] = \array_merge($parentQuery, [[\OM4\Vendor\ScssPhp\ScssPhp\Type::T_MEDIA_TYPE, [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, 'all']]], $childQuery);
                }
            }
        }
        return $this->multiplyMedia($env->parent, $childQueries);
     ...
     * @return array
     */
    protected function compactEnv(\OM4\Vendor\ScssPhp\ScssPhp\Compiler\Environment $env)
    {
     ...
     * @return \OM4\Vendor\ScssPhp\ScssPhp\Compiler\Environment
     */
    protected function pushEnv(\OM4\Vendor\ScssPhp\ScssPhp\Block $block = null)
    {
        $env = new \OM4\Vendor\ScssPhp\ScssPhp\Compiler\Environment();
        $env->parent = $this->env;
     ...
     */
    protected function set($name, $value, $shadow = \false, \OM4\Vendor\ScssPhp\ScssPhp\Compiler\Environment $env = null, $valueUnreduced = null)
    {
     ...
     * @param mixed                                 $valueUnreduced
     */
    protected function setExisting($name, $value, \OM4\Vendor\ScssPhp\ScssPhp\Compiler\Environment $env, $valueUnreduced = null)
    {
        $storeEnv = $env;
     ...
     * @param mixed                                 $valueUnreduced
     */
    protected function setRaw($name, $value, \OM4\Vendor\ScssPhp\ScssPhp\Compiler\Environment $env, $valueUnreduced = null)
    {
        $env->store[$name] = $value;
        if ($valueUnreduced) {
            $env->storeUnreduced[$name] = $valueUnreduced;
     ...
     * @return mixed|null
     */
    public function get($name, $shouldThrow = \true, \OM4\Vendor\ScssPhp\ScssPhp\Compiler\Environment $env = null, $unreduced = \false)
    {
        $normalizedName = $this->normalizeName($name);
        $specialContentKey = static::$namespaces['special'] . 'content';
        if (!isset($env)) {
     ...
     * @return boolean
     */
    protected function has($name, \OM4\Vendor\ScssPhp\ScssPhp\Compiler\Environment $env = null)
    {
     ...
     * @param \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $out
     */
    protected function importFile($path, \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock $out)
    {
    ...
    public function findImport($url)
    {
        ...
        if (!\preg_match('~\\.css$|^https?://|^//~', $url)) {
            ...
            if (!$isPartial) {
                $urls[] = \preg_replace('~[^/]+$~', 'OM4\\Vendor\\_', $url) . ($hasExtension ? '' : '.scss');
            }
            if (!$hasExtension) {
                $urls[] = "{$url}/index.scss";
        ...
        foreach ($this->importPaths as $dir) {
            if (\is_string($dir)) {
                ...
                foreach ($urls as $full) {
                    $found = [];
                    $separator = !empty($dir) && \substr($dir, -1) !== '/' && \substr($full, 0, 1) !== '/' ? '/' : '';
     ...
     *
     * @return \OM4\Vendor\ScssPhp\ScssPhp\Compiler
     *
     ...
     *
     * @throws \OM4\Vendor\ScssPhp\ScssPhp\Exception\CompilerException
     *
    ...
    public function error($msg, ...$args)
    {
        ...
        if (!$this->ignoreCallStackMessage) {
            ...
            $loc = isset($this->sourceNames[$this->sourceIndex]) ? $this->sourceNames[$this->sourceIndex] . " on line {$line}, at column {$column}" : "line: {$line}, column: {$column}";
            $msg = "{$msg}: {$loc}";
        ...
        return new \OM4\Vendor\ScssPhp\ScssPhp\Exception\CompilerException($msg);
    }
    /**
        ...
        if ($this->callStack) {
            foreach (\array_reverse($this->callStack) as $call) {
                if ($all || isset($call['n']) && $call['n']) {
                    $msg = '#' . $ncall++ . ' ' . $call['n'] . ' ';
                    $msg .= isset($this->sourceNames[$call[\OM4\Vendor\ScssPhp\ScssPhp\Parser::SOURCE_INDEX]]) ? $this->sourceNames[$call[\OM4\Vendor\ScssPhp\ScssPhp\Parser::SOURCE_INDEX]] : '(unknown file)';
                    $msg .= ' on line ' . $call[\OM4\Vendor\ScssPhp\ScssPhp\Parser::SOURCE_LINE];
                    $callStackMsg[] = $msg;
    ...
    protected function callScssFunction($func, $argValues)
    {
        ...
        $tmp = new \OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock();
        $tmp->lines = [];
        $tmp->children = [];
    ...
    {
        return \method_exists(\OM4\Vendor\ScssPhp\ScssPhp\Compiler::class, self::normalizeNativeFunctionName($name));
    }
    /**
    ...
    protected function sortNativeFunctionArgs($functionName, $prototypes, $args)
    {
        ...
        if (\in_array($functionName, ['libRgb', 'libRgba', 'libHsl', 'libHsla'])) {
            ...
            foreach ($args as $k => $arg) {
                if ($arg[1][0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST && \count($arg[1][2]) === 3) {
                    $last = \end($arg[1][2]);
                    if ($last[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_EXPRESSION && $last[1] === '/') {
                        \array_pop($arg[1][2]);
                        $arg[1][2][] = $last[2];
                        $arg[1][2][] = $last[3];
                        $args[$k] = $arg;
        ...
        foreach ($prototypes as $prototype) {
            ...
            foreach ($prototype as $i => $p) {
                ...
                if (\count($p)) {
                    ...
                    if ($p === 'null') {
                        ...
                        $default = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, 'null'];
                    } else {
                        if (\is_null($parser)) {
                            $parser = $this->parserFactory(__METHOD__);
                        }
            ...
            try {
                ...
                foreach ($prototype as $i => $p) {
                    ...
                    if ($finalArgs[$i] === [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, 'null']) {
                        $finalArgs[$i] = null;
                    }
                }
                // should we break if this prototype seems fulfilled?
            } catch (\OM4\Vendor\ScssPhp\ScssPhp\Exception\CompilerException $e) {
        ...
        if ($exceptionMessage && !$prototypeHasMatch) {
            if (\in_array($functionName, ['libRgb', 'libRgba', 'libHsl', 'libHsla'])) {
                ...
                foreach ($args as $arg) {
                    if ($arg[1][0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION_CALL && \in_array($arg[1][1], ['var'])) {
                        return null;
                    }
                }
            }
    ...
    protected function applyArguments($argDef, $argValues, $storeInEnv = \true, $reduce = \true)
    {
        ...
        foreach ((array) $argValues as $arg) {
            if (!empty($arg[0])) {
                ...
                if ($val[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST) {
                ...
                } elseif ($val[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MAP) {
        ...
        foreach ($args as $arg) {
            ...
            if ($isVariable) {
                ...
                $val = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, \is_null($splatSeparator) ? ',' : $splatSeparator, [], $isVariable];
    ...
    protected function coerceValue($value)
    {
        ...
        if (\is_numeric($value)) {
            return new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number($value, '');
        }
        if ($value === '') {
            return static::$emptyString;
        }
        $value = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, $value];
    ...
    {
        if ($item[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MAP) {
            return $item;
        }
    ...
    {
        if (isset($item) && $item[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST) {
        ...
        if (isset($item) && $item[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MAP) {
            $keys = $item[1];
            $values = $item[2];
            $list = [];
            for ($i = 0, $s = \count($keys); $i < $s; $i++) {
                ...
                switch ($key[0]) {
                    ...
                    case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NULL:
                        break;
                    default:
                        $key = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, $this->compileStringContent($this->coerceString($key))];
                        break;
                }
                $list[] = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, '', [$key, $value]];
            }
            return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, ',', $list];
        }
        return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, $delim, !isset($item) ? [] : [$item]];
    }
    /**
    ...
    protected function coerceColor($value, $inRGBFunction = \false)
    {
        switch ($value[0]) {
            ...
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST:
                if ($inRGBFunction) {
                    if (\count($value[2]) == 3 || \count($value[2]) == 4) {
                        $color = $value[2];
                        \array_unshift($color, \OM4\Vendor\ScssPhp\ScssPhp\Type::T_COLOR);
            ...
            case \OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD:
                    ...
                    if (\in_array($nofValues, [3, 4, 6, 8])) {
                        ...
                        if ($nbChannels === 4) {
                            if ($color[3] === 255) {
                                ...
                                $color[3] = \round($color[3] / 255, \OM4\Vendor\ScssPhp\ScssPhp\Node\Number::PRECISION);
                            }
                        }
                        \array_unshift($color, \OM4\Vendor\ScssPhp\ScssPhp\Type::T_COLOR);
                        return $color;
                ...
                if ($rgba = \OM4\Vendor\ScssPhp\ScssPhp\Colors::colorNameToRGBa($name)) {
                    return isset($rgba[3]) ? [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_COLOR, $rgba[0], $rgba[1], $rgba[2], $rgba[3]] : [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_COLOR, $rgba[0], $rgba[1], $rgba[2]];
                }
                return null;
        }
        return null;
     ...
     * @return integer|mixed
     */
    protected function compileColorPartValue($value, $min, $max, $isInt = \true, $clamp = \true, $modulo = \false)
    {
        if (!\is_numeric($value)) {
            if (\is_array($value)) {
                $reduced = $this->reduce($value);
                if (\is_object($reduced) && $value->type === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NUMBER) {
                    $value = $reduced;
                }
            }
            if (\is_object($value) && $value->type === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NUMBER) {
                $num = $value->dimension;
    ...
    {
        if ($value[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING) {
            return $value;
        }
        return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', [$this->compileValue($value)]];
    }
    /**
    ...
    public function assertString($value, $varName = null)
    {
        ...
        if ($value[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION) {
            $value = $this->coerceString($value);
        }
        if (!\in_array($value[0], [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, \OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD])) {
            $value = $this->compileValue($value);
    ...
    {
        if ($value[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NUMBER) {
            if (!empty($value[2]['%'])) {
                return $value[1] / 100;
            }
            return $value[1];
    ...
    public function assertMap($value)
    {
        ...
        if ($value[0] !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MAP) {
    ...
    public function assertInteger($value, $varName = null)
    {
        ...
        if (\round($value - \intval($value), \OM4\Vendor\ScssPhp\ScssPhp\Node\Number::PRECISION) > 0) {
            $var_display = $varName ? " \${$varName}:" : '';
    ...
    public function toHSL($red, $green, $blue)
    {
        ...
        return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_HSL, \fmod($h, 360), $s * 100, $l / 5.1];
    }
    /**
     * Hue to RGB helper
     *
    ...
    public function toRGB($hue, $saturation, $lightness)
    {
        ...
        $out = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_COLOR, $r, $g, $b];
        return $out;
    }
    // Built in functions
    ...
    {
        $functionReference = $this->reduce(\array_shift($args), \true);
        if (\in_array($functionReference[0], [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, \OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD])) {
            $name = $this->compileStringContent($this->coerceString($this->reduce($functionReference, \true)));
        ...
        if (!\in_array($functionReference[0], [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION_REFERENCE, \OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION])) {
        ...
        return $this->reduce([\OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION_CALL, $functionReference, $callArgs]);
    }
    protected static $libGetFunction = [['name'], ['name', 'css']];
    protected function libGetFunction($args)
    {
        $name = $this->compileStringContent($this->coerceString($this->reduce(\array_shift($args), \true)));
        $isCss = \false;
        if (\count($args)) {
            $isCss = $this->reduce(\array_shift($args), \true);
            $isCss = $isCss === static::$true ? \true : \false;
        }
        if ($isCss) {
            return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION, $name, [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, ',', []]];
        }
    ...
    protected function libIndex($args)
    {
        ...
        if ($list[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MAP || $list[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING || $list[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD || $list[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_INTERPOLATE) {
            $list = $this->coerceList($list, ' ');
        }
        if ($list[0] !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST) {
            return static::$null;
        }
        $values = [];
        foreach ($list[2] as $item) {
        ...
        switch (\count($args)) {
            case 1:
                if (!($color = $this->coerceColor($args[0], \true))) {
                    $color = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', [$funcName . '(', $args[0], ')']];
                }
                break;
            case 3:
                $color = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_COLOR, $args[0], $args[1], $args[2]];
                if (!($color = $this->coerceColor($color))) {
                    $color = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', [$funcName . '(', $args[0], ', ', $args[1], ', ', $args[2], ')']];
                }
                return $color;
            case 2:
                if ($color = $this->coerceColor($args[0], \true)) {
                    ...
                    if (\is_numeric($alpha)) {
                        ...
                        $color = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', [$funcName . '(', $color[1], ', ', $color[2], ', ', $color[3], ', ', $alpha, ')']];
                    }
                } else {
                    $color = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', [$funcName . '(', $args[0], ')']];
                }
            ...
            case 4:
            default:
                $color = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_COLOR, $args[0], $args[1], $args[2], $args[3]];
                if (!($color = $this->coerceColor($color))) {
                    $color = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', [$funcName . '(', $args[0], ', ', $args[1], ', ', $args[2], ', ', $args[3], ')']];
                }
                break;
        }
        return $color;
    ...
    protected function libIeHexStr($args)
    {
        ...
        return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', [\sprintf('#%02X%02X%02X%02X', $color[4], $color[1], $color[2], $color[3])]];
    }
    ...
    protected function libMix($args)
    {
        ...
        $w2 = 1.0 - $w1;
        $new = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_COLOR, $w1 * $first[1] + $w2 * $second[1], $w1 * $first[2] + $w2 * $second[2], $w1 * $first[3] + $w2 * $second[3]];
    ...
    protected function libHsl($args, $kwargs, $funcName = 'hsl')
    {
        $args_to_check = $args;
        if (\count($args) == 1) {
            if ($args[0][0] !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST || \count($args[0][2]) < 3 || \count($args[0][2]) > 4) {
                return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', [$funcName . '(', $args[0], ')']];
            }
            $args = $args[0][2];
            $args_to_check = $kwargs['channels'][2];
        }
        $hue = $this->compileColorPartValue($args[0], 0, 360, \false, \false, \true);
        ...
        foreach ($kwargs as $k => $arg) {
            if (\in_array($arg[0], [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION_CALL]) && \in_array($arg[1], ['min', 'max'])) {
                return null;
            }
        }
        foreach ($args_to_check as $k => $arg) {
            ...
            if ($k >= 2 && \count($args) === 4 && \in_array($arg[0], [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION_CALL, \OM4\Vendor\ScssPhp\ScssPhp\Type::T_FUNCTION]) && \in_array($arg[1], ['calc', 'env'])) {
                return null;
            }
        }
        $alpha = null;
        if (\count($args) === 4) {
            ...
            if (!\is_numeric($hue) || !\is_numeric($saturation) || !\is_numeric($lightness) || !\is_numeric($alpha)) {
                return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', [$funcName . '(', $args[0], ', ', $args[1], ', ', $args[2], ', ', $args[3], ')']];
            }
        } else {
            if (!\is_numeric($hue) || !\is_numeric($saturation) || !\is_numeric($lightness)) {
                return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', [$funcName . '(', $args[0], ', ', $args[1], ', ', $args[2], ')']];
            }
        }
    ...
    protected function libHue($args)
    {
        $color = $this->assertColor($args[0]);
        $hsl = $this->toHSL($color[1], $color[2], $color[3]);
        return new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number($hsl[1], 'deg');
    }
    ...
    protected function libSaturation($args)
    {
        $color = $this->assertColor($args[0]);
        $hsl = $this->toHSL($color[1], $color[2], $color[3]);
        return new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number($hsl[2], '%');
    }
    ...
    protected function libLightness($args)
    {
        $color = $this->assertColor($args[0]);
        $hsl = $this->toHSL($color[1], $color[2], $color[3]);
        return new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number($hsl[3], '%');
    }
    ...
    {
        $color = $this->assertColor($args[0]);
        $amount = \OM4\Vendor\ScssPhp\ScssPhp\Util::checkRange('amount', new \OM4\Vendor\ScssPhp\ScssPhp\Base\Range(0, 100), $args[1], '%');
    ...
    protected function libInvert($args)
    {
        ...
        if ($weight < 1) {
            return $this->libMix([$inverted, $color, [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_NUMBER, $weight]]);
        }
        return $inverted;
    }
    ...
    {
        $str = $args[0];
        if ($str[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING) {
            $str[1] = '';
        }
        return $str;
    }
    ...
    {
        $value = $args[0];
        if ($value[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING && !empty($value[1])) {
            return $value;
        }
        return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '"', [$value]];
    }
    protected static $libPercentage = ['number'];
    protected function libPercentage($args)
    {
        return new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number($this->coercePercent($args[0]) * 100, '%');
    }
    protected static $libRound = ['number'];
    protected function libRound($args)
    {
        $num = $args[0];
        return new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number(\round($num[1]), $num[2]);
    }
    protected static $libFloor = ['number'];
    protected function libFloor($args)
    {
        $num = $args[0];
        return new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number(\floor($num[1]), $num[2]);
    }
    protected static $libCeil = ['number'];
    protected function libCeil($args)
    {
        $num = $args[0];
        return new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number(\ceil($num[1]), $num[2]);
    }
    protected static $libAbs = ['number'];
    protected function libAbs($args)
    {
        $num = $args[0];
        return new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number(\abs($num[1]), $num[2]);
    }
    ...
    protected function libListSeparator($args)
    {
        ...
        if (!\in_array($args[0][0], [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, \OM4\Vendor\ScssPhp\ScssPhp\Type::T_MAP])) {
            return 'space';
        }
    ...
    protected function libMapKeys($args)
    {
        $map = $this->assertMap($args[0]);
        $keys = $map[1];
        return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, ',', $keys];
    }
    ...
    protected function libMapValues($args)
    {
        $map = $this->assertMap($args[0]);
        $values = $map[2];
        return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, ',', $values];
    }
    ...
    protected function libKeywords($args)
    {
        ...
        foreach ($args[0][2] as $name => $arg) {
            $keys[] = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, $name];
            $values[] = $arg;
        }
        return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_MAP, $keys, $values];
    }
    ...
    protected function libJoin($args)
    {
        ...
        if ($bracketed === static::$true) {
            ...
            $bracketed = \false;
        } elseif ($bracketed === [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, 'auto']) {
            $bracketed = 'auto';
        ...
        $res = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, $sep, \array_merge($list1[2], $list2[2])];
    ...
    protected function libAppend($args)
    {
        ...
        $list1 = $this->coerceList($list1, ' ', \true);
        $sep = $this->listSeparatorForJoin($list1, $sep);
        $res = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, $sep, \array_merge($list1[2], [$value])];
        if (isset($list1['enclosing'])) {
    ...
    protected function libZip($args)
    {
        ...
        $result = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, ',', $lists];
        if (!\is_null($firstList)) {
            foreach ($firstList[2] as $key => $item) {
                $list = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, '', [$item]];
                foreach ($args as $arg) {
                    if (isset($arg[2][$key])) {
                        $list[2][] = $arg[2][$key];
                    } else {
    ...
    {
        $num = $args[0];
        if ($num[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NUMBER) {
            return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '"', [$num->unitStr()]];
        }
        return '';
    }
    ...
    {
        $value = $args[0];
        return $value[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NUMBER && $value->unitless();
    }
    protected static $libComparable = [['number1', 'number2'], ['number-1', 'number-2']];
    protected function libComparable($args)
    {
        list($number1, $number2) = $args;
        if (!isset($number1[0]) || $number1[0] !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NUMBER || !isset($number2[0]) || $number2[0] !== \OM4\Vendor\ScssPhp\ScssPhp\Type::T_NUMBER) {
    ...
    protected function libStrIndex($args)
    {
        ...
        return $result === \false ? static::$null : new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number($result + 1, '');
    }
    ...
    protected function libStrInsert($args)
    {
        ...
        if ($index < 0) {
            $index = \OM4\Vendor\ScssPhp\ScssPhp\Util::mbStrlen($stringContent) + 1 + $index;
        }
        $string[2] = [\OM4\Vendor\ScssPhp\ScssPhp\Util::mbSubstr($stringContent, 0, $index), $insertContent, \OM4\Vendor\ScssPhp\ScssPhp\Util::mbSubstr($stringContent, $index)];
        return $string;
    }
    ...
    protected function libStrLength($args)
    {
        $string = $this->assertString($args[0], 'string');
        $stringContent = $this->compileStringContent($string);
        return new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number(\OM4\Vendor\ScssPhp\ScssPhp\Util::mbStrlen($stringContent), '');
    }
    ...
    protected function libCounter($args)
    {
        ...
        return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', ['counter(' . \implode(',', $list) . ')']];
    }
    ...
    protected function libRandom($args)
    {
        if (isset($args[0]) & $args[0] !== static::$null) {
            ...
            if (\round($n - \intval($n), \OM4\Vendor\ScssPhp\ScssPhp\Node\Number::PRECISION) > 0) {
                throw $this->error("Expected \$limit to be an integer but got {$n} for `random`");
            }
            return new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number(\mt_rand(1, \intval($n)), '');
        }
        $max = \mt_getrandmax();
        return new \OM4\Vendor\ScssPhp\ScssPhp\Node\Number(\mt_rand(0, $max - 1) / $max, '');
    }
    protected function libUniqueId()
    {
        ...
        $id += \mt_rand(0, 10) + 1;
        return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', ['u' . \str_pad(\base_convert($id, 10, 36), 8, '0', \STR_PAD_LEFT)]];
    }
    protected function inspectFormatValue($value, $force_enclosing_display = \false)
    {
        if ($value === static::$null) {
            $value = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD, 'null'];
        }
        $stringValue = [$value];
        if ($value[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST) {
            if (\end($value[2]) === static::$null) {
                \array_pop($value[2]);
                $value[2][] = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', ['']];
        ...
        return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', $stringValue];
    }
    ...
    protected function getSelectorArg($arg, $varname = null, $allowParent = \false)
    {
        ...
        if (!$this->checkSelectorArgType($arg)) {
            ...
            throw $this->error("Error:{$var_display} {$var_value} is not a valid selector: it must be a string," . " a list of strings, or a list of lists of strings");
        }
    ...
    {
        if ($arg[0] === \OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST && $maxDepth > 0) {
        ...
        if (!\in_array($arg[0], [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, \OM4\Vendor\ScssPhp\ScssPhp\Type::T_KEYWORD])) {
            return \false;
        }
        return \true;
    }
    ...
    {
        $selectors = $this->collapseSelectors($selectors, \true);
        return $selectors;
    }
    ...
    protected function libSimpleSelectors($args)
    {
        ...
        $listParts = [];
        foreach ($part as $p) {
            $listParts[] = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '', [$p]];
        }
        return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, ',', $listParts];
    }
    ...
    protected function libScssphpGlob($args)
    {
        ...
        foreach ($matches as $match) {
            ...
            $listParts[] = [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_STRING, '"', [$match]];
        }
        return [\OM4\Vendor\ScssPhp\ScssPhp\Type::T_LIST, ',', $listParts];
    }
}
