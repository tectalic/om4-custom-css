...
use OM4\Vendor\ScssPhp\ScssPhp\Block\AtRootBlock;
use OM4\Vendor\ScssPhp\ScssPhp\Block\CallableBlock;
use OM4\Vendor\ScssPhp\ScssPhp\Block\DirectiveBlock;
use OM4\Vendor\ScssPhp\ScssPhp\Block\EachBlock;
use OM4\Vendor\ScssPhp\ScssPhp\Block\ElseBlock;
use OM4\Vendor\ScssPhp\ScssPhp\Block\ElseifBlock;
use OM4\Vendor\ScssPhp\ScssPhp\Block\ForBlock;
use OM4\Vendor\ScssPhp\ScssPhp\Block\IfBlock;
use OM4\Vendor\ScssPhp\ScssPhp\Block\MediaBlock;
use OM4\Vendor\ScssPhp\ScssPhp\Block\NestedPropertyBlock;
use OM4\Vendor\ScssPhp\ScssPhp\Block\WhileBlock;
use OM4\Vendor\ScssPhp\ScssPhp\Compiler\CachedResult;
...
use OM4\Vendor\ScssPhp\ScssPhp\Exception\SassException;
use OM4\Vendor\ScssPhp\ScssPhp\Exception\SassScriptException;
use OM4\Vendor\ScssPhp\ScssPhp\Formatter\Compressed;
use OM4\Vendor\ScssPhp\ScssPhp\Formatter\Expanded;
use OM4\Vendor\ScssPhp\ScssPhp\Formatter\OutputBlock;
use OM4\Vendor\ScssPhp\ScssPhp\Logger\LoggerInterface;
use OM4\Vendor\ScssPhp\ScssPhp\Logger\StreamLogger;
use OM4\Vendor\ScssPhp\ScssPhp\Node\Number;
use OM4\Vendor\ScssPhp\ScssPhp\SourceMap\SourceMapGenerator;
use OM4\Vendor\ScssPhp\ScssPhp\Util\Path;
/**
...
class Compiler
{
    ...
    {
        $this->callStack[] = ['n' => $name, Parser::SOURCE_INDEX => $this->sourceIndex, Parser::SOURCE_LINE => $this->sourceLine, Parser::SOURCE_COLUMN => $this->sourceColumn];
        ...
        foreach ($queryList as $kql => $query) {
            ...
            foreach ($query as $kq => $q) {
                for ($i = 1; $i < \count($q); $i++) {
                    ...
                    if ($q[0] == Type::T_MEDIA_TYPE && (\strpos($value, '(') !== \false || \strpos($value, ')') !== \false || \strpos($value, ':') !== \false || \strpos($value, ',') !== \false)) {
                        $shouldReparse = \true;
                    }
...
EOL;
    ...
    protected function reduce($value, $inExp = \false)
    {
        ...
        switch ($value[0]) {
            case Type::T_EXPRESSION:
                ...
                if ($opName == 'div' && !$inParens && !$inExp && ($right[0] !== Type::T_NUMBER && isset($right[2]) && $right[2] != '' || $right[0] === Type::T_NUMBER && !$right->unitless())) {
        ...
        switch ($functionReference[1]) {
            ...
            case 'native':
                ...
                if (\in_array($name, ['min', 'max']) && \count($argValues) >= 1) {
                    $cssFunction = $this->cssValidArg([Type::T_FUNCTION_CALL, $name, $argValues], ['min', 'max', 'calc', 'env', 'var']);
     ...
     * @return mixed|null
     */
    public function get($name, $shouldThrow = \true, Environment $env = null, $unreduced = \false)
    {
        $normalizedName = $this->normalizeName($name);
        $specialContentKey = static::$namespaces['special'] . 'content';
        if (!isset($env)) {
    ...
    {
        if (!\in_array($formatterName, [Expanded::class, Compressed::class], \true)) {
        ...
        if ($this->callStack) {
            foreach (\array_reverse($this->callStack) as $call) {
                if ($all || isset($call['n']) && $call['n']) {
                    $msg = '#' . $ncall++ . ' ' . $call['n'] . ' ';
                    $msg .= isset($this->sourceNames[$call[Parser::SOURCE_INDEX]]) ? $this->getPrettyPath($this->sourceNames[$call[Parser::SOURCE_INDEX]]) : '(unknown file)';
        ...
        switch ($value[0]) {
            ...
            case Type::T_KEYWORD:
                ...
                if ($rgba = Colors::colorNameToRGBa($name)) {
                    return isset($rgba[3]) ? [Type::T_COLOR, $rgba[0], $rgba[1], $rgba[2], $rgba[3]] : [Type::T_COLOR, $rgba[0], $rgba[1], $rgba[2]];
                }
                return null;
        }
        return null;
    ...
    protected function libMix($args)
    {
        ...
        $weight2 = 1.0 - $weight1;
        $new = [Type::T_COLOR, $weight1 * $first[1] + $weight2 * $second[1], $weight1 * $first[2] + $weight2 * $second[2], $weight1 * $first[3] + $weight2 * $second[3]];
